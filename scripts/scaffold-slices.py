#!/usr/bin/env python3
"""Regenerate vertical-slice stub files for hecate-dns.
...
CMD_APPS = {
    "manage_repositories": {
        "description": "Create and retire git repositories on the mesh (CMD)",
        "slices": [
            {
                "slice":     "create_repository",
                "event":     "repository_created",
                "aggregate": "repository",
                "fields":    ["name", "steward", "default_branch", "visibility"],
                "http":      ("POST", "/api/git/repositories/create"),
            },
            {
                "slice":     "retire_repository",
                "event":     "repository_retired",
                "aggregate": "repository",
                "fields":    ["reason"],
                "http":      ("POST", "/api/git/repositories/retire"),
            },
            {
                "slice":     "rename_repository",
                "event":     "repository_renamed",
                "aggregate": "repository",
                "fields":    ["new_name"],
                "http":      ("POST", "/api/git/repositories/rename"),
            },
        ],
    },
    "manage_refs": {
        "description": "Move refs (branches, tags) on a repository (CMD)",
        "slices": [
            {
                "slice":     "advance_ref",
                "event":     "ref_advanced",
                "aggregate": "repository",
                "fields":    ["ref", "old_oid", "new_oid", "pusher"],
                "http":      ("POST", "/api/git/refs/advance"),
            },
            {
                "slice":     "retire_ref",
                "event":     "ref_retired",
...
QRY_APPS = {
    "query_repositories": {
        "description": "Repository read model (metadata, steward, visibility) (QRY)",
        "desks": [
            {"desk": "get_repository_by_id",   "kind": "byid", "http": ("GET", "/api/git/repositories/:repository_id")},
            {"desk": "get_repository_by_name", "kind": "page", "http": ("GET", "/api/git/repositories/by-name")},
            {"desk": "list_repositories_page", "kind": "page", "http": ("GET", "/api/git/repositories")},
        ],
    },
    "query_refs": {
        "description": "Ref read model (per-repository ref → oid map) (QRY)",
        "desks": [
            {"desk": "get_ref",            "kind": "page", "http": ("GET", "/api/git/refs/get")},
            {"desk": "list_refs_for_repo", "kind": "page", "http": ("GET", "/api/git/refs/by-repository")},
        ],
    },
}
...
PRJ_APPS = {
    "project_repositories": {
        "description": "Project repository events into the repositories read model (PRJ)",
        "projections": [
            {"event": "repository_created_v1", "table": "repositories", "source_app": "manage_repositories"},
            {"event": "repository_renamed_v1", "table": "repositories", "source_app": "manage_repositories"},
            {"event": "repository_retired_v1", "table": "repositories", "source_app": "manage_repositories"},
        ],
    },
    "project_refs": {
        "description": "Project ref events into the refs read model (PRJ)",
        "projections": [
            {"event": "ref_advanced_v1", "table": "refs", "source_app": "manage_refs"},
            {"event": "ref_retired_v1",  "table": "refs", "source_app": "manage_refs"},
        ],
    },
}
...
def main():
    print("Scaffolding hecate-git vertical slices into:")
    print(f"  {APPS}")
    generate_cmd_apps()
    generate_qry_apps()
    generate_prj_apps()
