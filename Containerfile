# Multi-stage Erlang build for hecate-git.
# Pushed to ghcr.io/hecate-services/hecate-git:latest + :semver.
...
WORKDIR /app
COPY --from=builder /build/_build/prod/rel/hecate-git/*.tar.gz /tmp/release.tar.gz
RUN tar xf /tmp/release.tar.gz && rm /tmp/release.tar.gz

# Realm cert mounts here; service socket mounts under /run/macula.
VOLUME ["/etc/hecate/secrets", "/var/lib/hecate-git"]
