%%% @doc Cowboy handler — POST /api/git/refs/retire.
-module(retire_ref_api).

-export([init/2, routes/0]).

routes() -> [{"/api/git/refs/retire", ?MODULE, []}].
...
handle(Req0, _State) ->
    case hecate_git_http:read_json_body(Req0) of
        {ok, Params, Req1} ->
            case retire_ref_v1:from_map(Params) of
                {ok, Cmd} ->
                    case maybe_retire_ref:dispatch(Cmd) of
                        ok ->
