%%% @doc Cowboy handler — POST /api/git/repositories/rename.
-module(rename_repository_api).

-export([init/2, routes/0]).

routes() -> [{"/api/git/repositories/rename", ?MODULE, []}].
...
handle(Req0, _State) ->
    case hecate_git_http:read_json_body(Req0) of
        {ok, Params, Req1} ->
            case rename_repository_v1:from_map(Params) of
                {ok, Cmd} ->
                    case maybe_rename_repository:dispatch(Cmd) of
                        ok ->
