%%% @doc Cowboy handler — GET /api/git/repositories/by-name.
-module(get_repository_by_name_api).

-export([init/2, routes/0]).

routes() -> [{"/api/git/repositories/by-name", ?MODULE, []}].

init(Req0, _State) ->
    case cowboy_req:method(Req0) of
        <<"GET">> ->
            Params = maps:from_list(cowboy_req:parse_qs(Req0)),
            case get_repository_by_name:handle(Params) of
                {ok, Items} ->
